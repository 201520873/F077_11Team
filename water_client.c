...
void *button_thd() {
	button_stop = 0;
	//button_stop = 0 while motor_thd send 1
	while (button_stop == 0) {
		state = GPIORead(PIN);
...
void *motor_thd() {
	...
	printf("motor_on: %d    button_on : %d    ", motor_on, button_on);
	//detect water
	if (motor_on == 1 && button_on == 1) {
		GPIOWrite(POUT2, 1);
	}
	//detect nothing
	else {
...
void *buzzer_thd() {
	int buzzer_on = atoi(msg) % 10; 
	//detect moving
	printf("buzzer_on : %d    \n", buzzer_on);
	if (buzzer_on == 1) {
		PWMWritePeriod(0, 1800000);
		PWMWriteDutyCycle(0, 1000000);
		usleep(250000);
		PWMWritePeriod(0, 4000000);
		usleep(250000);
	}
	//detect nothing
	else {
...
int main(int argc, char *argv[]) {
	...
	while(1) {                      
		...
		thr_id = pthread_create(&p_thread[2], NULL, buzzer_thd, NULL);
		if (thr_id < 0) {
			perror("thread create error : ");
			exit(0);
		}
